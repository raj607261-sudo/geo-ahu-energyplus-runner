...
export function normalizeDesign(input = {}) {
  const length = within(input.length, 2, 12, 6);
  const width = within(input.width, 2, 12, 4);
  const height = within(input.height, 2, 6, 3);
  const indoor = within(input.indoor, -25, 18, 8);
  const fieldTemp = within(input.fieldTemp, indoor, 55, 38);
  const dailyBatch = within(input.dailyBatch, 0, 10, 2.5);
  ...
  const ratedCapacityW = Math.max(2500, (productLoadW + envelopeLoadW + internalLoadW + respirationLoadW) * (1 + safety) * 1.15);
  const designFlow = Math.max(0.003, Math.min(0.01, (ratedCapacityW * 1.35) / (997 * 4180 * 5)));

  return {
    location: idfText(input.location, "Darbhanga Bihar"),
    crop: idfText(input.crop, "Fresh produce"),
    capacity: within(input.capacity, 0.1, 10, 10),
    length,
    width,
    height,
    indoor,
...
Schedule:Compact,Product Restocking,Any Number,Through: 12/31,For: AllDays,Until: 8:00,0,Until: ${pullEnd}:00,${x.productLoadW.toFixed(2)},Until: 24:00,0;
Schedule:Compact,Defrost Schedule,Fraction,Through: 12/31,For: AllDays,Interpolate:Average,Until: 5:00,0,Until: ${hourStamp(defrostEnd)},${defrostFraction},Until: 24:00,0;
Schedule:Compact,Dripdown Schedule,Fraction,Through: 12/31,For: AllDays,Interpolate:Average,Until: 5:00,0,Until: ${hourStamp(Math.min(24, defrostEnd + 0.5))},${defrostFraction},Until: 24:00,0;
...
Branch,
  ...
  ,
  Refrigeration:CompressorRack,Geo AHU Rack,Ground Loop Demand Inlet,Ground Loop Demand Outlet;
...
! Variant: ${variant === "geo" ? "water-cooled compressor rack + vertical ground heat exchanger" : "air-cooled compressor rack baseline"}
